...
class Tell(Plugin):
  ...
  def tell(self, bot, user_nick, receiver, message):
    if message.lower().startswith('!sage'):
      if (len(message.rstrip().split()) < 3) or (len(bot.split_by_nth(message.rstrip(), 2)[1]) <= 0):
        bot.send_message(receiver, user_nick + ": Benutzung: !sage <nick> <nachricht>")
      elif len(bot.split_by_nth(message, 2)[1]) > 144:
        bot.send_message(receiver, user_nick + ": Bitte bleibe unter 144 Zeichen.")
      else:
        ...
        bot.send_message(receiver, "Alles klar, mach ich.")
  ...
  def on_join(self, bot, user_nick, host, channel):
    ...
    for message in user_messages:
      bot.send_message("#" + channel, user_nick + ", " + message["sender"] + " liess dir am " + message["date"] + " um " + message["time"] + " Uhr ausrichten: " + message["message"])
  ...
  def on_privmsg(self, bot, user_nick, host, message):
    self.tell(bot, user_nick, user_nick, message)
